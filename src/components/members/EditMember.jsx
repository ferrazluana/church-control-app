...
const EditMember = () => {
    ...
    useEffect(() => {
        const fetchMinistries = async () => {
            const ministries = await getMinistriesActives(); // Fetch active ministries
    ...
    useEffect(() => {
        const fetchMemberData = async () => {
            ...
            setSelectedMinistries(memberMinistries.map(m => m.ministry_id)); // Ensure this is correct
        };
        fetchMemberData();
    }, [id]);
                            ...
                            {ministries.map(ministry => (
                                <label key={ministry.id} className="block mb-2">
                                    <input
                                        type="checkbox"
                                        value={ministry.id}
                                        checked={selectedMinistries.includes(ministry.id)} // This should mark the checkbox as checked if the ministry is selected
